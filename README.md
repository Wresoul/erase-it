...
- [x] Web-фронтенд
- [x] Полировка и финальное тестирование
...
```bash
docker build -t erase-it .
docker run -p 8000:8000 -v $(pwd)/ml/checkpoints:/app/ml/checkpoints erase-it
```

Монтируем `ml/checkpoints` как volume, чтобы не запекать веса SAM/генератора (сотни МБ)
в сам образ — их нужно скачать/обучить один раз локально (см. разделы выше) и переиспользовать.
...
29 тестов покрывают генератор масок, датасет, модели (генератор/дискриминатор), метрики
(PSNR/SSIM), обучение (включая честный overfit-тест и smoke-тест GAN), интеграцию SAM и
backend API — в том числе end-to-end сценарии `/segment` → `/inpaint` на изображениях
разных размеров и форматов (квадрат, широкий/узкий, нечётные размеры, объект у края,
клик по однородному фону без объекта).
